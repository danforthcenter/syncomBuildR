...
da <- function(df, col, predictors, zi_cutoff = 0.1){
  if(any(missing(df), missing(col), missing(predictors))){stop("df, col, and predictors must be specified")}
  if(mean(df[[col]]==0) > zi_cutoff){
    mod_type = "zinb"
  } else { mod_type = "nb"}
  
  if(mod_type == "zinb"){
    m<-pscl::zeroinfl(as.formula(paste0(col, "~", paste(predictors, collapse = "+"))), data = df, dist = "negbin", link="logit")
    s<-summary(m)
