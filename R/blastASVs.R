...
#' to make a blast database and seqs1 will be blasted against these.
...
blastASVs <- function(seqs1, seqs2, cutoff = 97.5, db = NULL, maxMatches = Inf) {
  if (is.null(db)) {
    ...
    db <- paste0("db/", timestamp, "_db")
    seqinr::write.fasta(seqs2, names(seqs2),
                        file.out = paste0(timestamp, ".fasta"))
    rBLAST::makeblastdb(file = paste0(timestamp, ".fasta"),
                        db_name = db, dbtype = "nucl")
  }
  db_obj <- rBLAST::blast(db)
  res <- do.call(rbind, lapply( seq_along(seqs1), function(i) {
    sq <- seqs1[[i]]
    ...
    if(nrow(res)>=1){
      ...
      res$match_rank <- rank(res$pident, ties.method = "average")
    } else {
      res <- structure(list(seq1_id = names(seqs1)[i], db_id = "NONE", pident = 0, 
