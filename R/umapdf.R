...
#' 
#' @param df Dataframe to ordinate. Generally UMAP works best on principal components, so this is assumed to be output from \link{\code{pcadf}}.
#' @param cols columns to reduce dimensions of. Can be specified with names or positions. Defaults to all column names starting with "pc" then numbers for use with \link{\code{pcadf}}.
...
#' @param ... Additional arguments passed to \link{\code{uwot::umap}}. The n_neighbors, n_components, and pca arguments are potentially commonly useful.
#' @keywords pca, umap
#' 
#' @import ggplot2
#' @import uwot
...
umapdf<-function(df=NULL, cols=NULL, color=NULL, returnData=T, ...){
  # df=pdf$data; cols=NULL; color=c("tissue", "genotype"); returnData=T
  if(is.null(cols)){cols=which(grepl("^pc[0-9]+",colnames(df)))
  ...
  colnames(coords)<-c(paste0("umap",1:ncol(coords)))
  umap.df<-cbind(df[, -cols], coords)
  if(length(color)>1){
    umap.df[[paste0(color, collapse=".")]] = interaction(umap.df[,c(color)])
    color=paste0(color, collapse=".")
  }
  if(is.null(color)){umap.df$dummyVariableForColor = 1; color="dummyVariableForColor"}
  p<-ggplot2::ggplot(umap.df, ggplot2::aes(x=umap1, y=umap2, color = .data[[color]]))+
  ...
  if(returnData){return(list("data"=umap.df, "plot"=p))}else{return(p)}
}
